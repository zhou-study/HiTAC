function [labelsC_out, info] = third(cluster_neigh, d, labelsC, k, pointE,varargin)
% THIRD 仅依据"互为第二近邻"的簇对，按 d 从小到大依次合并（只改簇级 labelsC）
%
% Inputs
%   cluster_neigh : C×K int，簇的邻接表（升序；第 2 列为"第二近邻"，1-based）
%   d             : C×C double，簇间距离（对称；d(i,i)=0）
%   labelsC       : C×1 int，"同值同簇"的簇标签（不是父指针/不是 1..C 的必须条件）
%   k             : 目标簇数（当根数<=k 或无候选对时停止）
%
% Name-Value（可选）
...
%
% Outputs
%   labelsC_out : C×1 int，合并后的簇级标签（以"根簇索引"表示；同簇同值）
...
% -------- 基础检查 --------
labelsC = labelsC(:);
C = size(cluster_neigh, 1);
assert(size(d,1)==C && size(d,2)==C, 'd 的尺寸必须是 C×C');
assert(numel(labelsC)==C, 'labelsC 的长度必须等于簇数 C');
...
if size(cluster_neigh,2) < 2
    ...
    info = struct('cand_pairs_super', [], 'cand_dist', [], 'merges', [], ...
                  'stoppedReason', 'no_second_neighbor_column', ...
                  'finalK', curK);
    return;
end

sec = cluster_neigh(:, 2);                        % 第二近邻
valid = sec>=1 & sec<=C & sec ~= (1:C)';          % 合法且不是自己
pairs = [];
for i = 1:C
...
if isempty(pairs)
    labelsC_out = arrayfun(@(i) findroot(parent, i), (1:C)');
    info = struct('cand_pairs_super', [], 'cand_dist', [], 'merges', [], ...
                  'stoppedReason', 'no_mutual_second_neighbors', ...
                  'finalK', curK);
    return;
end
